...
rem		java -classpath %JARS%;%CLASS_PATH%; net.ion.radon.upgrade.InstallRunner -config:./resource/install/window_install.xml -target:start_server
	...
	if /i "%2" == "register" (
		...
		%JAVASERVICE% -install AradonServer %JVMDLL% -Djava.class.path=%JARS%;%CLASS_PATH%; -Daradon.home.dir=%ARADON_HOME% -Mms256M -Xmx512M -start net.ion.radon.ServerRunner -params -config:%ARADON_HOME%/resource/config/aradon-config.xml -action:restart -out %ARADON_HOME%/resource/logs/server_out.log -err %ARADON_HOME%/resource/logs/server_err.log %MODE%
	) else if /i "%2" == "start" (
