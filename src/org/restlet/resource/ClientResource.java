 ...
 * This class changes the semantics of the {@link UniformResource#getRequest()} and {@link UniformResource#getResponse()} methods. Since a clientResource may receive severals responses for a single request (in case of interim response), the {@link #getResponse()} method returns the last received response object. The Request object returned by the {@link #getRequest()} is actually a prototype which
 * is cloned (except the representation) just before the {@link #handle()} method is called.<br>
 * Users must be aware that by most representations can only be read or written once. Some others, such as {@link StringRepresentation} stored the entity in memory which can be read several times but has the drawback to consume memory.<br>
 * Concurrency note: instances of the class are not designed to be shared among several threads. If thread-safety is necessary, consider using the lower-level {@link Client} class instead.
 * 
...
public class ClientResource extends UniformResource {
	...
	/**
	 * Creates a client resource that proxy calls to the given Java interface into Restlet method calls. It basically creates a new instance of {@link ClientResource} and invokes the {@link #wrap(Class)} method.
	 * 
	 * @param <T>
	 * @param context
	 *            The context.
	...
	/**
	 * Releases the resource by stopping any connector automatically created and associated to the "next" property (see {@link #getNext()} method.
	 */
	@Override
	 ...
	 * @throws ResourceException
	 */
	private <T> T handle(Method method, Class<T> resultClass) throws ResourceException {
	...
	protected <T> T handle(Method method, Object entity, Class<T> resultClass) throws ResourceException {
		...
		result = toObject(handle(method, (entity == null) ? null : toRepresentation(entity, clientInfo.getPreferredVariant(variants, getMetadataService())), clientInfo), resultClass);
		return result;
	}
