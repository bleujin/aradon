...
public class NettyEventSourceChannelHandler extends SimpleChannelUpstreamHandler {
	...
	public NettyEventSourceChannelHandler(Executor executor, EventSourceHandler handler, ChannelHandlerContext ctx, UncaughtExceptionHandler exceptionHandler, NettyHttpRequest nettyHttpRequest, UncaughtExceptionHandler ioExceptionHandler, NettyEventSourceConnection eventSourceConnection,
		...
		executor.execute(new Runnable() {
			public void run() {
				try {
					...
					exceptionHandler.uncaughtException(Thread.currentThread(), AradonRuntimeException.fromException(e1, e.getChannel()));
				}
			}
		});
	}
