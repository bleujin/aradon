...
public class NettyWebSocketChannelHandler extends SimpleChannelUpstreamHandler {
	...
	public NettyWebSocketChannelHandler(Executor executor, WebSocketHandler handler, ChannelHandlerContext ctx, UncaughtExceptionHandler exceptionHandler, NettyHttpRequest nettyHttpRequest, UncaughtExceptionHandler ioExceptionHandler, NettyWebSocketConnection webSocketConnection, HttpRequest req,
			HttpResponse res) {
		...
		try {
			...
			exceptionHandler.uncaughtException(Thread.currentThread(), new AradonRuntimeException(e));
		}
	}
		...
		executor.execute(new Runnable() {
			public void run() {
				try {
					...
					exceptionHandler.uncaughtException(thread, AradonRuntimeException.fromException(e1, e.getChannel()));
				}
			}
		});
	}
			...
			executor.execute(new Runnable() {
				public void run() {
					try {
						...
						exceptionHandler.uncaughtException(Thread.currentThread(), AradonRuntimeException.fromException(throwable, e.getChannel()));
					}
				}
			});
		}
