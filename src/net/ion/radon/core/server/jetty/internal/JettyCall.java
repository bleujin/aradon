...
public class JettyCall extends ServerCall {
	...
	public List<Certificate> getCertificates() {
		Certificate[] certificateArray = (Certificate[]) getConnection().getRequest().getAttribute("javax.servlet.request.X509Certificate");
	...
	public String getCipherSuite() {
		return (String) getConnection().getRequest().getAttribute("javax.servlet.request.cipher_suite");
	}
	...
	public Series<Header> getRequestHeaders() {
		...
		if (!this.requestHeadersAdded) {
			...
			for (Enumeration<String> names = getConnection().getRequestFields().getFieldNames(); names.hasMoreElements();) {
				headerName = names.nextElement();

				for (Enumeration<String> values = getConnection().getRequestFields().getValues(headerName); values.hasMoreElements();) {
	...
	public Integer getSslKeySize() {
		Integer keySize = (Integer) getConnection().getRequest().getAttribute("javax.servlet.request.key_size");
	...
	public String getSslSessionId() {
		Object sessionId = getConnection().getRequest().getAttribute("javax.servlet.request.ssl_session_id");
