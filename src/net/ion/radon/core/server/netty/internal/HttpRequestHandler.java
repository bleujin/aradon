...
import net.ion.radon.core.server.netty.HttpsServerHelper;
...
/**
 * HTTP request handler implementation. Pass HTTP requests to Restlet and gather HTTP response from Restlet and provide it back to the client.
 * 
		...
		if ((!request.isChunked()) || lastChunk) {
			...
			if (restletResponse != null) {
				...
				try {
					...
					for (Header header : httpCall.getResponseHeaders()) {
