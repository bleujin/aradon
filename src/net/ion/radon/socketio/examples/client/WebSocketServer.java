...
public abstract class WebSocketServer implements Runnable, WebSocketListener {
	...
	// CONSTANTS ///////////////////////////////////////////////////////////////
	/**
	 * The value of <var>handshake</var> when a Flash client requests a policy file on this server.
	 */
	...
	/**
	 * Creates a WebSocketServer that will attempt to listen on port <var>port</var>, and comply with <tt>WebSocketDraft</tt> version <var>draft</var>.
	 * 
	 * @param port
	 *            The port number this server should listen on.
	 * @param draft
	 *            The version of the WebSocket protocol that this server instance should comply to.
	 */
			...
			try {
				...
				while (i.hasNext()) {
					...
					if (key.isValid() && key.isWritable()) {
						WebSocket conn = (WebSocket) key.attachment();
						if (conn.handleWrite()) {
							conn.socketChannel().register(selector, SelectionKey.OP_READ, conn);
						}
					}
				}
				...
				for (WebSocket conn : this.connections) {
					...
					if (conn.hasBufferedData()) {
						conn.socketChannel().register(selector, SelectionKey.OP_READ | SelectionKey.OP_WRITE, conn);
					}
				}
			} catch (IOException ex) {
				ex.printStackTrace();
	...
	/**
	 * Called by a {@link WebSocket} instance when a client connection has finished sending a handshake. This method verifies that the handshake is a valid WebSocket cliend request. Then sends a WebSocket server handshake if it is valid, or closes the connection if it is not.
	 * 
	 * @param conn
	 *            The {@link WebSocket} instance who's handshake has been recieved.
	 * @param handshake
	 *            The entire UTF-8 decoded handshake from the connection.
	 * @return True if the client sent a valid WebSocket handshake and this server successfully sent a WebSocket server handshake, false otherwise.
	 * @throws IOException
	 *             When socket related I/O errors occur.
