...
public abstract class WebSocketClient implements Runnable, WebSocketListener {
	...
	/**
	 * Calls <var>close</var> on the underlying SocketChannel, which in turn closes the socket connection, and ends the client socket thread.
	 * 
	 * @throws IOException
	...
	private void sendHandshake() throws IOException {
		...
		if (this.key3 != null) {
			conn.socketChannel().write(new ByteBuffer[] { ByteBuffer.wrap(request.getBytes(WebSocket.UTF8_CHARSET)), ByteBuffer.wrap(this.key3) });
		} else {
	 ...
	 * 
	 * @param conn
	 *            The {@link WebSocket} instance who's handshake has been recieved. In the case of <tt>WebSocketClient</tt>, this.conn == conn.
	 * @param handshake
	 *            The entire UTF-8 decoded handshake from the connection.
	 * @return <var>true</var> if <var>handshake</var> is a valid WebSocket server handshake, <var>false</var> otherwise.
	 * @throws IOException
	 *             When socket related I/O errors occur.
	...
	public boolean onHandshakeRecieved(WebSocket conn, String handshake, byte[] reply) throws IOException, NoSuchAlgorithmException {
		...
		if (this.draft == WebSocketDraft.DRAFT76) {
			...
			byte[] challenge = new byte[] { (byte) (this.number1 >> 24), (byte) ((this.number1 << 8) >> 24), (byte) ((this.number1 << 16) >> 24), (byte) ((this.number1 << 24) >> 24), (byte) (this.number2 >> 24),
					(byte) ((this.number2 << 8) >> 24), (byte) ((this.number2 << 16) >> 24), (byte) ((this.number2 << 24) >> 24), this.key3[0], this.key3[1], this.key3[2], this.key3[3], this.key3[4], this.key3[5], this.key3[6],
					this.key3[7] };
