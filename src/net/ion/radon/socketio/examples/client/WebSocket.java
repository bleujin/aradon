...
public final class WebSocket {
	...
	// INSTANCE PROPERTIES /////////////////////////////////////////////////////
	/**
	 * The <tt>SocketChannel</tt> instance to use for this server connection. This is used to read and write data to.
	 */
	...
	/**
	 * Lock object to ensure that data is sent from the bufferQueue in the proper order.
	 */
	 ...
	 * 
	 * @param socketChannel
	 *            The <tt>SocketChannel</tt> instance to read and write to. The channel should already be registered with a Selector before construction of this object.
	 * @param bufferQueue
	 *            The Queue that we should use to buffer data that hasn't been sent to the client yet.
	 * @param listener
	...
	/**
	 * @return True if all of the text was sent to the client by this thread. False if some of the text had to be buffered to be sent later.
	 */
	...
	/**
	 * @return True if all data has been sent to the client, false if there is still some buffered.
	 */
	...
	private void recieveHandshake() throws IOException, NoSuchAlgorithmException {
		...
		if ((h.length >= 20 && h[h.length - 20] == CR && h[h.length - 19] == LF && h[h.length - 18] == CR && h[h.length - 17] == LF)) {
			completeHandshake(new byte[] { h[h.length - 16], h[h.length - 15], h[h.length - 14], h[h.length - 13], h[h.length - 12], h[h.length - 11], h[h.length - 10], h[h.length - 9], h[h.length - 8], h[h.length - 7], h[h.length - 6],
		...
		} else if ((h.length >= 12 && h[h.length - 12] == CR && h[h.length - 11] == LF && h[h.length - 10] == CR && h[h.length - 9] == LF) && new String(this.remoteHandshake.array(), UTF8_CHARSET).contains("Sec-WebSocket-Key1")) {
			completeHandshake(new byte[] { h[h.length - 8], h[h.length - 7], h[h.length - 6], h[h.length - 5], h[h.length - 4], h[h.length - 3], h[h.length - 2], h[h.length - 1] });

			// Consider Draft 75, and the Flash Security Policy
			// Request edge-case.
		} else if ((h.length >= 4 && h[h.length - 4] == CR && h[h.length - 3] == LF && h[h.length - 2] == CR && h[h.length - 1] == LF) && !(new String(this.remoteHandshake.array(), UTF8_CHARSET).contains("Sec"))
